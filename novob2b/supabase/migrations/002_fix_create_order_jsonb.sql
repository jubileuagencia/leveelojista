-- ============================================================================
-- FIX: create_order_validated - JSON → JSONB
-- ============================================================================
-- Problema: PostgREST envia arrays/objetos como JSONB, mas a funcao original
-- usava JSON. Nao existe cast implicito jsonb → json no PostgreSQL,
-- causando erro 400 (Bad Request) ao chamar via supabase.rpc().
-- Solucao: DROP + CREATE com parametro JSONB.
-- Execucao: Copiar e colar no Supabase SQL Editor > Run
-- ============================================================================

-- Drop versoes existentes (JSON e JSONB, caso ambas existam)
DROP FUNCTION IF EXISTS create_order_validated(UUID, TEXT, JSON);
