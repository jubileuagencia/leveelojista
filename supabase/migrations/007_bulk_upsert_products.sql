-- ============================================================================
-- Migration 007: RPC bulk_upsert_products
-- ============================================================================
-- Descricao: RPC para upsert em massa de produtos via Google Sheets.
--   - Recebe array JSONB de produtos
--   - display_id NULL = INSERT (novo produto)
--   - display_id existente = UPDATE
--   - Resolve category_name → category_id automaticamente
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Resolver category_name → category_id
      v_cat_id := NULL;
      ...
      IF v_display_id IS NOT NULL THEN
        ...
        ELSE
          -- display_id fornecido mas nao existe — inserir com display_id especifico
        ...
        END IF;
      ELSE
        -- INSERT (novo produto, display_id gerado pela sequence)
  ...
  -- Sincronizar sequence com o maior display_id existente
  -- Evita conflito quando a sequence alcanca um display_id inserido manualmente
  IF v_inserted > 0 THEN
    PERFORM setval(
      'products_display_id_seq',
      GREATEST(nextval('products_display_id_seq') - 1, (SELECT COALESCE(MAX(display_id), 0) FROM products)),
      TRUE
    );
  END IF;
...
-- Permissao: apenas service_role pode executar (chamado via Apps Script)
REVOKE ALL ON FUNCTION bulk_upsert_products(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION bulk_upsert_products(JSONB) FROM anon;
REVOKE ALL ON FUNCTION bulk_upsert_products(JSONB) FROM authenticated;
-- service_role tem acesso automatico via SECURITY DEFINER
